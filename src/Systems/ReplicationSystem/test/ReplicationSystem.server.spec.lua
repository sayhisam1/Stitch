...
return function()
	...
	describe("Component added", function()
		...
		it("should update replicate on component update", function()
			...
			world.entityManager:updateComponent(testComponent, instance, {
				foo = "baz",
				momma = "mia",
			})
			Promise.fromEvent(RunService.Heartbeat):await()
			...
			world.entityManager:setComponent(testComponent, instance, {})
			Promise.fromEvent(RunService.Heartbeat):await()
			Promise.fromEvent(RunService.Heartbeat):await()
	...
	describe("Component removed", function()
		it("should remove replicate folder", function()
			...
			expect(world.entityManager:getComponent(testComponent, instance)).to.be.ok()
			expect(instance:FindFirstChild(replicatedFolderName)).to.be.ok()
			world.entityManager:removeComponent(testComponent, instance)
			Promise.fromEvent(RunService.Heartbeat):await()
			Promise.fromEvent(RunService.Heartbeat):await()
